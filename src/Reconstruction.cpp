...
#pragma omp parallel for num_threads(Nthread)
  ...
  else if(is_RECON == 5) {//sol is primitive variables
    VEC h(N_x);
    Sol CON_tmp(N_x);
    bU U1, U2, U3, U4;
    double h1, h2, h3, h4;
...
#pragma omp parallel for num_threads(Nthread)
    for(u_int i = 1; i < N_x; ++i) {//perform characteristic decomposition
      if(i == 1) {
        Chara_MAT(sol[i-1], Gamma[i-1], RMAT1, LMAT1);
        Chara_MAT(sol[i], Gamma[i], RMAT2, LMAT2);
        h1 = h[0];
        h2 = h[i-1];
        h3 = h[i];
        h4 = h[i+1];

        U1 = multiply(CON_tmp[0], LMAT1);
        U2 = multiply(CON_tmp[i-1], LMAT1);
        U3 = multiply(CON_tmp[i], LMAT1);
        for(u_int d = 0; d < 3; ++d) {
          ReconL_Con[i][d] = ENO2_CELL_R(h1, h2, h3, U1[d], U2[d], U3[d]);
        }
        U2 = multiply(CON_tmp[i-1], LMAT2);
        U3 = multiply(CON_tmp[i], LMAT2);
        U4 = multiply(CON_tmp[i+1], LMAT2);
        for(u_int d = 0; d < 3; ++d) {
          ReconR_Con[i][d] = ENO2_CELL_L(h2, h3, h4, U2[d], U3[d], U4[d]);
        }
        ReconL_Con[i] = multiply(ReconL_Con[i], RMAT1);
        ReconR_Con[i] = multiply(ReconR_Con[i], RMAT2);
        ReconL_Pri[i] = Con2Pri(ReconL_Con[i], Gamma[i-1]);
        ReconR_Pri[i] = Con2Pri(ReconR_Con[i], Gamma[i]);
      }
      else if(i == N_x-1) {
        ...
        U2 = multiply(CON_tmp[i-1], LMAT2);
        U3 = multiply(CON_tmp[i], LMAT2);
        U4 = multiply(CON_tmp[N_x-1], LMAT2);
        for(u_int d = 0; d < 3; ++d) {
          ReconR_Con[i][d] = ENO2_CELL_L(h2, h3, h4, U2[d], U3[d], U4[d]);
        }
    ...
    //deal with left BD
    {
      Chara_MAT(sol[0], Gamma[0], RMAT2, LMAT2);
      h2 = h[0];
      h3 = h[0];
      h4 = h[1];
      U2 = multiply(CON_tmp[0], LMAT2);
      U3 = multiply(CON_tmp[0], LMAT2);
      U4 = multiply(CON_tmp[1], LMAT2);
      for(u_int d = 0; d < 3; ++d) {
        ReconR_Con[0][d] = ENO2_CELL_L(h2, h3, h4, U2[d], U3[d], U4[d]);
      }
      ReconR_Con[0] = multiply(ReconR_Con[0], RMAT2);
      ReconR_Pri[0] = Con2Pri(ReconR_Con[0], Gamma[0]);
    }
    //deal with right BD
    {
      Chara_MAT(sol[N_x-1], Gamma[N_x-1], RMAT1, LMAT1);
      h1 = h[N_x-2];
      h2 = h[N_x-1];
      h3 = h[N_x-1];
      U1 = multiply(CON_tmp[N_x-2], LMAT1);
      U2 = multiply(CON_tmp[N_x-1], LMAT1);
      U3 = multiply(CON_tmp[N_x-1], LMAT1);
      for(u_int d = 0; d < 3; ++d) {
        ReconL_Con[N_x][d] = ENO2_CELL_R(h1, h2, h3, U1[d], U2[d], U3[d]);
      }
      ReconL_Con[N_x] = multiply(ReconL_Con[N_x], RMAT1);
      ReconL_Pri[N_x] = Con2Pri(ReconL_Con[N_x], Gamma[N_x-1]);
    }
    if(BD == 1) {//outflow
      ReconL_Con[0] = ReconR_Con[0];
