...
void Lagranian1D::Euler_forward_LF(double dt, double alpha, VEC& mesh) {
  ...
  //std::cout << ReconL_Con << std::endl;
  //std::cout << ReconR_Con << std::endl;
  //std::cout << ReconL_Pri << std::endl;
  //std::cout << ReconR_Pri << std::endl;
...
#pragma omp parallel for num_threads(Nthread)
  for(u_int i = 0; i < N_x; ++i) {
    Con[i] = Con[i] - dt*(FLUX[i+1] - FLUX[i]) / Di[i];
    Con[i][0] = (mesh[i+1] - mesh[i]) / Di[i];
...
void Lagranian1D::Euler_forward_TS(const double dt, VEC& mesh) {
  Reconstruction(Con, mesh, ReconL_Con, ReconR_Con, ReconL_Pri, ReconR_Pri);
  cal_flux_TS(ReconL_Con, ReconR_Con, ReconL_Pri, ReconR_Pri, FLUX, us);
  move_mesh(mesh, us, dt, mesh);
#pragma omp parallel for num_threads(Nthread)
  for(u_int i = 0; i < N_x; ++i) {
    Con[i] -= dt*(FLUX[i+1] - FLUX[i]) / Di[i];
    Pri[i] = Con2Pri(Con[i], Gamma[i]);
  }
  update_cs(cs);
}
